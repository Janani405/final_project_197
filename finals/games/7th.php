...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO quiz (Game_Type, Level, Score) VALUES (?, ?, ?)";
...
<body>
  ...
  <div class="center-content">
    <div class="images">
      <img src="../images/could.png" alt="Elephant Image">
    </div>
    <form action="" method="POST">
      <h3>Who am I:</h3>
      <div class="radio-group">
        <input type="radio" id="ice" name="animal" value="ice">
        <label for="ice">ICE-CREAM</label>
        <input type="radio" id="elephant" name="animal" value="elephant">
        <label for="elephant">ELEPHANT</label>
        <input type="radio" id="cloud" name="animal" value="cloud">
        <label for="cloud">CLOUD</label>
      </div>
      <button type="submit" class="submit-btn">Submit</button>
      <button type="button" class="skip-btn" onclick="window.location.href='8th.php';">Skip</button>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($selectedAnimal === 'cloud') {
        echo "
        <script>
        Swal.fire({
            icon: 'success',
            title: 'Correct!',
            text: 'You selected CLOUD!'
        }).then((result) => {
            if (result.isConfirmed) {
                window.location.href = '8th.php'; // Replace with your target HTML file
            }
        });
        </script>";
    } else {
