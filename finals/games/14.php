...
<body>
  ...
  <div class="center-content">
    <div class="images">
      <img src="../images/fox-removebg-preview.png" alt="PIZZA Image">
    </div>
    <form action="" method="POST">
      <h3>Who am I:</h3>
      <div class="radio-group">
        <input type="radio" id="fox" name="animal" value="fox">
        <label for="fox">FOX</label>
        <input type="radio" id="chef" name="animal" value="chef">
        <label for="chef">CHEF</label>
        <input type="radio" id="tree" name="animal" value="tree">
        <label for="tree">TREE</label>
      </div>
      <button type="submit" class="submit-btn">Submit</button>
      <button type="button" class="skip-btn" onclick="window.location.href='15.php';">Skip</button>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($selectedAnimal === 'fox') {
        ...
        Swal.fire({
            icon: 'success',
            title: 'Correct!',
            text: 'You selected FOX!'
        }).then((result) => {
            if (result.isConfirmed) {
                window.location.href = '15.php'; // Replace with your target HTML file
            }
        });
        </script>";
    } else {
